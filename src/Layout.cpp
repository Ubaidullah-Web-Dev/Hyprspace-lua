...
void CHyprspaceWidget::updateLayout() {
  ...
  static auto PGAPSINDATA =
      CConfigValue<Config::IComplexConfigValue>("general:gaps_in");
  static auto PGAPSOUTDATA =
      CConfigValue<Config::IComplexConfigValue>("general:gaps_out");
  if (!PGAPSINDATA.good() || !PGAPSOUTDATA.good())
    return;

  const auto PGAPSINBASE = PGAPSINDATA.ptr();
  const auto PGAPSOUTBASE = PGAPSOUTDATA.ptr();
  if (!PGAPSINBASE || !PGAPSOUTBASE ||
      PGAPSINBASE->getDataType() != Config::CVD_TYPE_CSS_VALUE ||
      PGAPSOUTBASE->getDataType() != Config::CVD_TYPE_CSS_VALUE)
    return;
  ...
  if (active) {
    ...
    for (auto &ws :
         g_pCompositor->getWorkspaces()) { // HACK: recalculate other workspaces
