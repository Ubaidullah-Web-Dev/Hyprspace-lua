...
void renderLayerStub(PHLLS pLayer, PHLMONITOR pMonitor, CBox rectOverride,
  ...
  if (!pLayer->m_mapped || pLayer->m_readyToDelete || !pLayer->m_layerSurface ||
      !pLayer->wlSurface() || !pLayer->wlSurface()->resource())
    return;
    ...
    if (ws == owner->m_activeWorkspace) {
      if (Config::workspaceBorderSize >= 1 &&
        ...
        renderBorder(curWorkspaceBox,
                     Config::CGradientValueData(Config::workspaceActiveBorder),
      ...
      if (Config::workspaceBorderSize >= 1 &&
        ...
        renderBorder(
            curWorkspaceBox,
            Config::CGradientValueData(Config::workspaceInactiveBorder),
            Config::workspaceBorderSize);
      }
      if (!Config::disableBlur) {
